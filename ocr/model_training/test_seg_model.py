...
def _load_seg_model_in_handler(handler_module : ModuleType, model_path : Path) -> None :
    ...
    if hasattr(handler_module, "MODEL_PATH") :
        setattr(handler_module, "MODEL_PATH", model_path)
        return
...
def test_seg_model(
    ...
    for item in dataset :
        ...
        page_score = segmentation_metric(gt_lines, pred_lines, image, item)
...
def test_seg_models_concurrently(
    ...
    processes = []
    for (model_path, handler_path), queue in zip(normalized, queues) :
        p = mp.Process(
    ...
    for p in processes :
        p.join()
    if one_line : print(line + f"| {iteration} / {pages_per_model + 1}")

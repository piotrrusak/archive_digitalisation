...
class PairReport:
    ...
    px_region_pct: float
    line_height_mean: float | None
    line_height_median: float | None
    line_height_std: float | None
    spacing_mean: float | None
...
def print_summary(reports):
    ...
    if bl_vals:
        mn, md, mx = min(bl_vals), float(np.median(bl_vals)), max(bl_vals)
        print(f"Baseline%   min/median/max: {mn:.4f} / {md:.4f} / {mx:.4f}")

    if rg_vals:
        mn, md, mx = min(rg_vals), float(np.median(rg_vals)), max(rg_vals)
        print(f"Region%     min/median/max: {mn:.4f} / {md:.4f} / {mx:.4f}")

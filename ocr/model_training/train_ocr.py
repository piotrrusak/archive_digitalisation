# THIS IS FIRST DRAFT OF TRAINING SCRIPT FOR OCR MODEL.
# IT IS NOT WORKING CORRECTLY YET AND NEEDS FURTHER IMPROVEMENTS.
...
def _load_metric_sibling(model_path: Path) -> tuple[str, float, bool] | None:
    for suf in (".json", ".metrics.json", ".report.json"):
        ...
        if p.exists():
            try:
                ...
                def add_if_num(key: str, higher_is_better: bool, data=data, candidates=candidates):
                    v = data.get(key, None)
            ...
            if lines:
                with (run_dir / "_debug_gt_lines.txt").open("w", encoding="utf-8") as f:
                    for i, line_text in enumerate(lines):
                        f.write(f"[{i}] {line_text}\n")
                print(f"[debug] Saved Example GT lines to {run_dir / '_debug_gt_lines.txt'}")
        else:
            print("[debug] Failed to read targets from batch (unexpected format).")
...
def promote(
    ...
    if not candidates:
        raise FileNotFoundError(f"No mlmodel files found matching model_*.mlmodel in {run_dir}")
